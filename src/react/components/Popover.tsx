...
export const PopoverTrigger = memo(
  forwardRef<HTMLButtonElement, PopoverTriggerProps>(
    ({ children, asChild = false, className, toggle = true }, ref) => {
...
export const PopoverContent = memo(
  forwardRef<HTMLDivElement, PopoverContentProps>(
    (
      ...
      if (!shouldRender || !mounted || !portalContainer) return null
      ...
      return createPortal(
        <div
          ...
          className={cn(
            `z-50 min-w-[8rem] overflow-hidden ${borderRadius.default} border border-border bg-card p-2 shadow-lg`,
            'origin-top',
