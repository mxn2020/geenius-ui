...
export const Slider = forwardRef<HTMLDivElement, SliderProps>(
  (
    ...
    const handleThumbPointerDown = useCallback(
      (event: React.MouseEvent | React.TouchEvent, thumbIndex: number) => {
        if (disabled) return
        event.preventDefault()
    ...
    const handleTrackPointerDown = useCallback(
      (event: React.MouseEvent | React.TouchEvent) => {
        ...
        const closestThumbIndex = value.reduce((closest, thumbValue, index) => {
          ...
          return Math.abs(newValue - thumbValue) <
            Math.abs(newValue - closestValue)
            ? index
            : closest
        }, 0)
        ...
        >
          ...
          {value.map((thumbValue, index) => (
            <div
              key={index}
              className={cn(
                'absolute block h-5 w-5 rounded-full border-2 border-primary bg-card shadow-md transition-colors',
              ...
              style={
                isVertical
                  ? {
                      bottom: `${getThumbPosition(thumbValue)}%`,
                      transform: 'translateY(50%)',
                    }
                  : {
                      left: `${getThumbPosition(thumbValue)}%`,
