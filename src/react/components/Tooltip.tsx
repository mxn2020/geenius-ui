...
const LegacyTooltip = memo(
  forwardRef<ElementRef<'div'>, LegacyTooltipProps>(
    (
      {
        content,
        children,
...
export const TooltipContent = memo(
  forwardRef<ElementRef<'div'>, TooltipContentProps>(
    (
      { side = 'top', align = 'center', className, children, ...props },
      ref,
    ) => {
      const ctx = useContext(TooltipContext)
      if (!ctx) return null
      ...
      return (
        <div
          ...
          className={cn(
            `absolute z-50 px-3 py-2 text-sm ${borderRadius.default} shadow-lg border border-border/70 bg-popover text-popover-foreground`,
