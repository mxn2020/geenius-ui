...
const Select = memo(function Select({
  ...
  const onTypeaheadChar = useCallback(
    (char: string) => {
      ...
      const startIndex = activeValue
        ? Math.max(0, order.indexOf(activeValue))
        : 0
      const candidates = order
        .slice(startIndex)
        .concat(order.slice(0, startIndex))
      const labels = labelMap.current

      const hit = candidates.find((v) =>
        (labels.get(v) || v).toLowerCase().startsWith(q),
      )
...
const SelectTrigger = memo(
  forwardRef<HTMLButtonElement, SelectTriggerProps>(
    ({ children, className, placeholder, asChild = false }, ref) => {
      const {
        open,
        setOpen,
        value,
...
const SelectValue = memo(
  forwardRef<HTMLSpanElement, SelectValueProps>(
  ...
  forwardRef<HTMLDivElement, SelectContentProps>(
    (
            ...
            const nextIndex =
              idx === -1
                ? delta === 1
                  ? 0
                  : order.length - 1
                : (idx + delta + order.length) % order.length
      ...
      useEffect(() => {
        ...
        if (!viewport) return
        const itemEl = viewport.querySelector<HTMLElement>(
          `[data-select-item='${CSS.escape(activeValue)}']`,
        )
        itemEl?.scrollIntoView({ block: 'nearest' })
      }, [open, activeValue, viewportRef])

      // Use animated open for proper exit animations
      const { shouldRender, dataState, animationStyle } = useAnimatedOpen(
        open,
        {
      ...
      // Render the portal always (so contentRef is attached for measurement),
      // but hide it with opacity:0 + pointer-events:none until position is
      // computed. This prevents the dropdown from flashing at (0,0).
      const positioned = pos !== null

      const style: CSSProperties = {
        position: 'fixed',
        left: positioned ? pos.x : 0,
        top: positioned ? pos.y : 0,
        width: triggerRef.current?.getBoundingClientRect().width,
        // Before position is computed: fully invisible, no interactions
      ...
      return createPortal(
        <div
          ref={mergeRefs(setRef(contentRef), ref)}
          style={style}
...
const SelectLabel = memo(
  forwardRef<HTMLDivElement, SelectLabelProps>(
...
const SelectItem = memo(
  forwardRef<HTMLDivElement, SelectItemProps>(
    (
      { children, value, disabled = false, className, label: labelProp },
      ref,
    ) => {
      const {
        value: selectedValue,
