...
const SpinnerIcon = ({ className }: { className?: string }) => (
  <div
    className={cn(
      'inline-block animate-spin rounded-full border-b-2 border-foreground mb-4',
      className,
    )}
  />
)
...
const PulseIcon = ({ className }: { className?: string }) => (
  <div className={cn('bg-primary rounded-full animate-pulse', className)} />
)

export const Loading = memo(
  forwardRef<ElementRef<'div'>, LoadingProps>(
    (
      {
        size = 'md',
        variant = 'spinner',
      ...
      const spinnerContent = (
        ...
        >
          <Icon
            className={variant !== 'dots' ? sizeClasses[size] : undefined}
          />
          {showMessage && (
      ...
      if (fullScreen) {
        return (
          <div
            ref={ref}
            className={cn(
              'flex items-center justify-center min-h-screen bg-surface',
              className,
            )}
            {...props}
          >
...
export const LoadingSkeleton = memo(
  forwardRef<ElementRef<'div'>, LoadingSkeletonProps>(
    (
      {
        className,
        width = 'w-full',
