...
const useResizableContext = () => {
  ...
  if (!context) {
    throw new Error(
      'Resizable compound components must be used within a ResizablePanelGroup',
    )
  }
  return context
}
...
export const ResizablePanelGroup = memo(
  forwardRef<HTMLDivElement, ResizablePanelGroupProps>(
    (
      {
        children,
      ...
      const registerPanel = useCallback(
        (id: string, constraint: PanelConstraint) => {
          setPanels((prev) => {
          ...
          setPanels((prev) => {
            ...
            if (panel) {
              ...
              if (panel.constraint.min !== undefined) {
                constrainedSize = Math.max(
                  constrainedSize,
                  panel.constraint.min,
                )
              }
              if (panel.constraint.max !== undefined) {
                constrainedSize = Math.min(
                  constrainedSize,
                  panel.constraint.max,
                )
              }
...
export const ResizablePanel = memo(
  forwardRef<HTMLDivElement, ResizablePanelProps>(
    (
      {
        children,
        defaultSize = 50,
      ...
      const [panelId] = useState(
        () => providedId || `panel-${Math.random().toString(36).substr(2, 9)}`,
      )
...
export const ResizableHandle = memo(
  forwardRef<HTMLDivElement, ResizableHandleProps>(
    (
      { className, withHandle = false, disabled = false, onDragging, ...props },
      ref,
    ) => {
      const { direction, startResize, activeHandleId, updatePanelSize } =
        useResizableContext()
      const [handleId] = useState(
        () => `handle-${Math.random().toString(36).substr(2, 9)}`,
      )
      const [isDragging, setIsDragging] = useState(false)
      const containerRef = useRef<HTMLDivElement | null>(null)
      const startPosRef = useRef(0)
      const panelBeforeRef = useRef<{ id: string; initialSize: number } | null>(
        null,
      )
      const panelAfterRef = useRef<{ id: string; initialSize: number } | null>(
        null,
      )
      ...
      const handlePointerDown = useCallback(
        (clientX: number, clientY: number) => {
          ...
          const panelBefore = allPanels[
            Math.floor(handleIndex / 2)
          ] as HTMLElement
          const panelAfter = allPanels[
            Math.ceil(handleIndex / 2)
          ] as HTMLElement
          ...
          const panelBeforeSize = parseFloat(
            panelBefore.getAttribute('data-panel-size') || '50',
          )
          const panelAfterSize = parseFloat(
            panelAfter.getAttribute('data-panel-size') || '50',
          )

          panelBeforeRef.current = {
            id: panelBeforeId,
            initialSize: panelBeforeSize,
          }
          panelAfterRef.current = {
            id: panelAfterId,
            initialSize: panelAfterSize,
          }
      ...
      useEffect(() => {
        ...
        const handleMove = (
          ...
          const newBeforeSize =
            panelBeforeRef.current.initialSize + deltaPercent
