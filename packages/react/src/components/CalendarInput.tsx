...
export const buildTimezoneOptions = (timezones?: string[]) => {
  const now = new Date()
  const zones =
    timezones && timezones.length > 0 ? timezones : getSupportedTimezones()
  ...
  const unique = options.filter(
    (opt): opt is { value: string; offset: number; offsetStr: string } => {
      if (!opt) return false
...
const formatDisplayFromParts = (
  ...
  if (format24Hour) timeStr = `${parts.hours}:${parts.minutes}`
    ...
    timeStr = `${parts.hours.padStart(2, '0')}:${parts.minutes} ${parts.meridiem}`
...
>((props, ref) => {
  ...
  const [activePane, setActivePane] = useState<'date' | 'time'>(() =>
    mode === 'time' ? 'time' : 'date',
  )

  const resolvedDefaultTimezone = defaultTimezone || defaultUserTimezone()
  const activeDisplayTimezone =
    displayTimezone || value?.timezone || resolvedDefaultTimezone

  const [cursorDate, setCursorDate] = useState<Date>(() => {
    if (value?.timestamp)
      return getDateInZone(value.timestamp, activeDisplayTimezone)
  ...
  const toTimeParts = useCallback(
    (timestamp: number, tz: string) =>
      getTimePartsInZone(timestamp, tz, format24Hour),
    [format24Hour],
  )

  const [timeParts, setTimeParts] = useState(() => {
    if (value?.timestamp)
      return toTimeParts(value.timestamp, activeDisplayTimezone)
    return {
      hours: format24Hour ? '12' : '12',
      minutes: '00',
      meridiem: 'PM' as 'AM' | 'PM',
    }
  })

  const [inputValue, setInputValue] = useState<string>(() => {
    if (!value?.timestamp) return ''
    const parts = getTimePartsInZone(
      value.timestamp,
      activeDisplayTimezone,
      format24Hour,
    )
    ...
    if (timezoneOptions && timezoneOptions.length > 0) {
      ...
      const enriched = timezoneOptions
        .map((opt) => {
          try {
            const offsetMinutes =
              typeof opt.offsetMinutes === 'number'
                ? opt.offsetMinutes
                : getOffsetMinutes(now, opt.value)
            const offsetString =
              opt.offsetString ?? toOffsetString(offsetMinutes)
            return {
              value: opt.value,
  ...
  useEffect(() => {
    if (value?.timestamp) {
      ...
      setTimeParts(parts)
      setInputValue(
        formatDisplayFromParts(displayDate, parts, mode, format24Hour),
      )
      ...
      setTimeParts(
        getTimePartsInZone(
          now,
          displayTimezone || resolvedDefaultTimezone,
          format24Hour,
        ),
      )
    }
  ...
  const mergeWithTime = useCallback(
    (base: Date, tz: string): CalendarValueWithTimezone => {
      ...
      if (format24Hour) {
        resolvedHours = Math.min(
          23,
          Math.max(0, isNaN(hoursInt) ? 0 : hoursInt),
        )
      } else {
  ...
  const emitValue = useCallback(
    (
      ...
      if (payload?.timestamp) {
        ...
        const parts =
          nextParts ||
          getTimePartsInZone(payload.timestamp, activeTimezone, format24Hour)

        setCursorDate(nextCursor || displayDate)
        setTimeParts(parts)
        setInputValue(
          formatDisplayFromParts(
            nextCursor || displayDate,
            parts,
            mode,
            format24Hour,
          ),
  ...
  useEffect(() => {
    ...
    if (
      !timezoneChoices.some((opt) => opt.value === activeTimezone) &&
      timezoneChoices[0]
    ) {
  ...
  const parseAndCommit = useCallback(
    (raw: string) => {
      ...
      let nextParts:
        | { hours: string; minutes: string; meridiem: 'AM' | 'PM' }
        | undefined
      ...
      if (mode === 'date') {
        ...
        if (parsedTime) {
          const base = value?.timestamp
            ? getDateInZone(value.timestamp, activeTimezone)
            : new Date()
  ...
  const applyTemplateTime = useCallback(
    (template: Date) => {
      ...
      const payload = buildDateFromParts(
        {
          year: nextCursor.getFullYear(),
          month: nextCursor.getMonth(),
          day: nextCursor.getDate(),
          hour: template.getHours(),
          minute: template.getMinutes(),
        },
        activeTimezone,
      )
  ...
  const normalizeTimeInput = useCallback(
    (_field: 'hours' | 'minutes') => () => {
      ...
      if (!format24Hour) {
        if (timeParts.meridiem === 'PM' && resolvedHours !== 12)
          resolvedHours += 12
        if (timeParts.meridiem === 'AM' && resolvedHours === 12)
          resolvedHours = 0
      }
      ...
      const nextParts = getTimePartsInZone(
        payload.timestamp!,
        activeTimezone,
        format24Hour,
      )
  ...
  const toggleTimeFormat = useCallback(() => {
    ...
    setTimeParts((prev) => {
      ...
      if (newFormat) {
        ...
        // 24h -> 12h
        const hours12 =
          hoursInt === 0 ? 12 : hoursInt > 12 ? hoursInt - 12 : hoursInt
  ...
  const toggleMeridiem = useCallback(() => {
    ...
    const base = value?.timestamp
      ? getDateInZone(value.timestamp, activeTimezone)
      : cursorDate
    const hours12 = Math.min(
      12,
      Math.max(1, parseInt(timeParts.hours, 10) || 12),
    )
    const minutes = Math.min(
      59,
      Math.max(0, parseInt(timeParts.minutes, 10) || 0),
    )
  ...
  const quickTimeOptions = useMemo(() => {
    const steps = Array.from(
      new Set([0, timeStepMinutes, timeStepMinutes * 2, 60, 120]),
    ).filter((n) => n >= 0)
    const now = new Date()
    return steps.map((step) => ({
      label: step === 0 ? 'Now' : `+${step}m`,
      date: addMinutes(now, step),
    }))
  }, [timeStepMinutes])
        ...
        <PopoverTrigger asChild toggle={false}>
          <div
            className={cn(
              ...
              error &&
                'border-destructive/50 focus-within:ring-destructive focus-within:border-destructive',
              className,
            )}
                              ...
                              ? buildDateFromParts(
                                  {
                                    ...
                                    month: option.date.getMonth(),
                                    day: option.date.getDate(),
                                    hour: 0,
                                    minute: 0,
                                  },
                                  activeTimezone,
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-border bg-card">
              {showTips ? (
                <div className="text-xs text-muted-foreground">
                  Fast typing: enter dates as `12/31/2025` or times as `930a`,
                  `18:00`
                </div>
              ) : (
                <span />
