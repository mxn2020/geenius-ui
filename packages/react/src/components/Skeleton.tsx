...
export const Skeleton = memo(
  forwardRef<HTMLDivElement, SkeletonProps>(
    (
      { className, variant = 'default', width, height, animate = true },
      ref,
    ) => {
...
export const SkeletonAvatar = memo(
  ({ size = 'md', className }: SkeletonAvatarProps) => {
    const sizeClasses = {
      sm: 'w-8 h-8',
      md: 'w-10 h-10',
      lg: 'w-12 h-12',
...
export const SkeletonForm = memo(
  ({ fields = 4, hasSubmit = true, className }: SkeletonFormProps) => {
    return (
      <div className={cn('space-y-4', className)}>
        ...
        {hasSubmit && <Skeleton height={40} className="w-32" />}
      </div>
    )
  },
)
...
export const SkeletonMetricCard = memo(
  ({ hasIcon = true, hasTrend = true, className }: SkeletonMetricCardProps) => {
    return (
      <div className={cn('space-y-4', className)}>
